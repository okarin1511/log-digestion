...
function countEndpointHits(logFilePath: string) {
        ...
        let timeSeriesData = {} as { [key: string]: number };
        let endpointMapper = {} as { [key: string]: number };
        let statusCodeMapper = {} as { [key: string]: number };
...
async function askQuestion(logFiles: string[]) {
    const answer = await inquirer.prompt([
        {
            type: "list",
            name: "name",
